...
from agent_gen.librarian import MANIFEST_FILE
...
class TestCliCommands(unittest.TestCase):
    ...
    def test_e2e_lifecycle(self):
        with self.runner.isolated_filesystem():
            ...
            self.assertTrue(os.path.exists("agents/test-agent/agent-manifest.json"))
            self.assertTrue(os.path.exists("agents/test-agent/skills"))
            
            # Create a file so the directory is not empty and gets archived
            Path("agents/test-agent/skills/my-skill.md").touch()

            # Describe (update description)
            result = self.runner.invoke(cli, ["describe", "test-agent", "--desc", "A test agent"])
            self.assertEqual(result.exit_code, 0)
            with open("agents/test-agent/agent-manifest.json") as f:
            ...
            self.assertEqual(result.exit_code, 0)
            self.assertFalse(os.path.exists("agents/test-agent"))
    ...
    def test_retrofit_claude_canonical(self):
        with self.runner.isolated_filesystem():
            # Scaffold a mock Claude structure
            ...
            # Verify it was mapped to AgentFactory standard (in-place)
    ...
    def test_import_skill_directory(self):
        with self.runner.isolated_filesystem():
            ...
            # 4. Verify
            target_skill_dir = Path("agents/target-agent/skills/super-skill")
            self.assertTrue(target_skill_dir.exists())
            self.assertTrue((target_skill_dir / "ability.py").exists())
            
            with open("agents/target-agent/agent-manifest.json") as f:
                manifest = json.load(f)
    ...
    def test_import_skill_zip(self):
        with self.runner.isolated_filesystem():
            ...
            # 4. Verify
            target_skill_dir = Path("agents/target-agent/skills/zip-skill")
            self.assertTrue(target_skill_dir.exists())
